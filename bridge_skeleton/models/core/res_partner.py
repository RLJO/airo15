...
class ResPartner(models.Model):
    ...
    fiscal_position_name = fields.Char(string="fiscal position name", compute="_compute_fiscal_position_name", store=True)

    @api.depends('property_account_position_id')
    def _compute_fiscal_position_name(self):
        for rec in self:
            if rec.property_account_position_id:
                rec.fiscal_position_name = rec.property_account_position_id.name
            else:
                rec.fiscal_position_name = ''
    ...
    def customer_array(self, data):
        ...
        if data.get('street_name'):
            name_lst = data['street_name'].split(' ')
            if len(name_lst)>1:
                data['street_number'] = name_lst[-1]
                data['street_name'] = ''.join(name_lst[0:-1])
        return data
